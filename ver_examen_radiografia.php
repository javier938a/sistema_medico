...
include ("_core.php");
    ...
    $query = _query("SELECT e_r.*, e_p.fecha_lectura, e_p.id_examen_paciente FROM examen_radiografia AS e_r, examen_paciente AS e_p WHERE e_r.id_examen_paciente ='$id_examen_paciente' AND e_p.id_examen_paciente ='$id_examen_paciente'");
    $num = _num_rows($query);
    if($num>0)
    {
        $datos = _fetch_array($query);
        $resultado = $datos["resultado"];
        $id_examen = $datos["id_radiografia"];
...
<div class="modal-header">
	...
	<h4 class="modal-title">Resultados: Radiografía</h4>
</div>

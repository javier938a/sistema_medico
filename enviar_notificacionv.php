...
function initial() 
{
	...
	$fecha= $_REQUEST['fecha'];
  	...
  	$sql0 = _query("SELECT sms, ws FROM empresa, notificaciones_varias WHERE id_empresa='1'");
  	$datos = _fetch_array($sql0);
  	$sms = $datos["sms"]; 
  	$ws = $datos["ws"]; 
  	if($tipo =="Teléfono")
  	{
    	$sql="SELECT  n.id, n.id_paciente, n.enviado, n.mensaje, p.tel1 as contacto FROM notificaciones_varias as n, paciente as p WHERE n.fecha='$fecha' AND n.tipo='$tipo' AND p.id_paciente=n.id_paciente ORDER BY n.id, n.mensaje ASC";
	}
	else
	{
		$sql="SELECT  n.id, n.id_paciente, n.enviado, n.mensaje, p.email as contacto FROM notificaciones_varias as n, paciente as p WHERE n.fecha='$fecha' AND n.tipo='$tipo' AND p.id_paciente=n.id_paciente ORDER BY n.id, n.mensaje ASC";
	}
    $result=_query($sql);
			        ...
			        <table class="table table-condensed table-striped">
			        	<thead class="thead-inverse">
							<tr class="bg-success">
				        		...
				        		<th class="col-lg-6">Mensaje</th>
				        		<th class="col-lg-1">Contacto</th>
				        		<th class='text-center col-lg-1'><p><div class='checkbox i-checks' id="all"><label><input id='chk' name='chk' type='checkbox'></label></div></p></th>
				        	</tr>
			        	</thead>
			        	<tbody id="datos">
			        	<?php
							...
							while($row = _fetch_array($result))
							{
								echo "<tr>
										...
										if(!$row["enviado"])
										{
											echo "<p><div class='checkbox i-checks includes'><label><input id='myCheckboxes' name='myCheckboxes' type='checkbox' value='".$row["id"]."'></label></div></p>";
										}
										else
										{
